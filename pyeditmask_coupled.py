...
class SimpleMPLCanvas(object):
    ...
    def make_navbar(self):
        ...
        self.navbar.SetPosition((680,0)) # this is now working (MT 2016)
...
#                        self.OnSettings)
	...
	if (maskr[j+1,i] == 1 and (maskr[j,i-1] + maskr[j+1,i+1] == 0 or \
	    			   maskr[j,i+1] + maskr[j+1,i-1]==0) \
	    or maskr[j-1,i] == 1 and (maskr[j,i-1] + maskr[j-1,i+1] == 0 or \
	    			      maskr[j,i+1] + maskr[j-1,i-1]==0) \
	    or maskr[j,i-1] == 1 and (maskr[j+1,i-1] + maskr[j-1,i] == 0 or \
	      			      maskr[j+1,i] + maskr[j-1,i-1]==0) \
	    or maskr[j,i+1] == 1 and (maskr[j+1,i+1] + maskr[j-1,i] == 0 or \
	    			      maskr[j+1,i] + maskr[j-1,i+1]==0)):
		mask = 0
	...
	m = nf.variables['h'][line-3:line+3,col-3:col+3]
	l,c=np.where(m>20.)
	...
	print np.mean(m[l,c])
	self.plot_maskcrit(grd,self.lon0,line,col)      
	#ax.set_title('h(' + str(line)+ ','+str(col)+') was set to '+ str(np.mean(m[l,c]))+'m' ,loc='Left')	
	...
	button = evt.button
	    ...
	    for i in range(a):
			...
			idx = np.where(grd.maskr[i-1:i+1,j-1:j+1]==1)
		    	...
		    	grd.h[i,j] = np.mean(grd.h[i-1:i+1,j-1:j+1])
			grd.maskcrit[i,j] = 1
			print np.mean(grd.h[i-1:i+1,j-1:j+1])	     
            ax.clear() 
	    self.plot_grid(grd,self.lon0)
	    ax.set_xlim([self.lo.min(), self.lo.max()])
	    ax.set_ylim([self.la.min(), self.la.max()])        
